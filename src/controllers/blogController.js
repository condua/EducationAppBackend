...
exports.getBlogById = async (req, res) => {
  try {
    ...
    // Kiểm tra xem yêu cầu có phải từ Facebook bot hay không (hoặc các bot khác)
    const isBotRequest =
      req.headers["user-agent"] &&
      req.headers["user-agent"].toLowerCase().includes("facebook");

    if (isBotRequest) {
      // Render HTML cho bot (Facebook, Google, ...)
      ...
      // Trả về HTML đã render cho bot
      return res.status(200).send(htmlContent);
    } else {
      // Nếu không phải yêu cầu từ bot (frontend, ứng dụng khác), trả về JSON
